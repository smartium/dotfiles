...
g_main_colour = "0x0a0a0a"
g_alt_colour = "0xffffff"
...
function conky_ring_stats(cr)
    ...
    local function setup_cpu_text(cr, tset, font_colour, font_alt_colour, value)
        local str = ''
        local thread_num = tonumber(tset.text)

        if (thread_num >= 1 and thread_num <= 3) or (thread_num >= 7 and thread_num <= 9) then
    ...
    local function setup_text(cr, value, pt, tset)
        ...
        elseif pt.name == 'cpu' then
            setup_cpu_text(cr, tset, font_colour, font_alt_colour, value)
        else
...
text_settings = {
    ...
    -- clock
    { text = 'HH', show = true, x = 95, y = 636 },
    { text = 'MM', show = true, x = 109, y = 636 },
    { text = 'SS', show = false, x = 142, y = 670 },
...
function draw_elements(line_sketches_toggle)
    ...
    local function draw_logo(cr)
        local w, h = 0, 0
        local imagefile = home_dir .. "/.config/conky/hybrid/images/distro-1b.png"
